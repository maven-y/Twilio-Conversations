...
class MessageListViewModel(
    ...
    init {
        ...
        viewModelScope.launch {
            try {
                val client = ConversationsClientWrapper.INSTANCE.getConversationsClient()
                client.getConversation(conversationSid) { conversation ->
                    _conversation.postValue(conversation)
                }
            } catch (e: Exception) {
                // Handle error
                Timber.e(e, "Failed to get conversation")
            }
        }
    }
    ...
    fun getConversationParticipants() = conversationsRepository.getConversationParticipants(conversationSid)

    suspend fun getAccessToken(): String {
        return try {
            conversationsRepository.getToken()
        } catch (e: Exception) {
